...
export async function getQuestion(req, res, next) {
  try {
    ...
    const currentUser = await resolveCurrentUser(req);
    const question = await Question.findById(id).populate("user", "name email").lean();
    if (!question) {
    ...
    res.json({
      question: toQuestionDTO(question, { likesCount, likedByUser })
    });
  } catch (err) {
    next(err);
  }
...
export async function createQuestion(req, res, next) {
  try {
    ...
    const hydrated = await Question.findById(question._id).populate("user", "name email").lean();
    res.status(201).json({
      question: toQuestionDTO(hydrated, { likesCount: 0, likedByUser: false })
    });
  } catch (err) {
    next(err);
  }
